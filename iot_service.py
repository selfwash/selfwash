...
def _encrypt_enc1(inner_payload: dict, kid: str, ts: str, nonce: str) -> str:
    ...
    iv = os.urandom(12)
    business_json = json.dumps(inner_payload, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
    wrapped_plaintext = {
        "body_b64": base64.b64encode(business_json).decode("utf-8"),
    }
    plaintext = json.dumps(wrapped_plaintext, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
...
def _decrypt_enc1_response(payload: dict) -> dict:
    ...
    # VMT wraps business payload in {"body_b64":"..."} before encryption.
    if isinstance(decoded, dict) and isinstance(decoded.get("body_b64"), str):
        try:
            business = base64.b64decode(decoded["body_b64"]).decode("utf-8")
            decoded = json.loads(business)
        except Exception as exc:
            raise ValueError("Decrypted ENC1 response body_b64 is invalid") from exc
...
def _build_signature(
    ...
    body_sha256_hex = _sha256_hex(enc1_json_body.encode("utf-8"))
    string_to_sign = "\n".join(
        [
            "POST",
            command_path,
